...
export async function POST(req: Request) {
    let email, password, role, country;
            ...
            if (authError.message?.toLowerCase().includes("already been registered") ||
                ...
                if (dbError) {
                    if (dbError.code === "23505") { // unique_violation
                        return Response.json({ message: "User already exists. Access instructions have been resent." });
                    }
                    return Response.json({ error: dbError.message }, { status: 400 });
                }

                return Response.json({ message: "Existing user linked and access instructions sent!" });
            }
